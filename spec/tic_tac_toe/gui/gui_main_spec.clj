  ...
  (context "play game edn"
    (it "launches quil with new game-id, filepath, and sets db type"
      (with-redefs [launch-quil (stub :launch-quil)
        ...
        (should-have-invoked :launch-quil {:with [{:game-id 13, :filepath "src/tic_tac_toe/game_logs/in_progress/gui/game-13.edn", :current-screen :resume-selection, :db :edn, :second-ai-level nil, :mode nil, :first-ai-level nil, :game-state :in-progress, :human? nil, :ui :gui, :player :x, :board nil}]})))
    )

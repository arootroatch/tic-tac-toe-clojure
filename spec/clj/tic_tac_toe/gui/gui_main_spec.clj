...
(describe "ttt-gui"
  ...
  (context "replay sql"
    ...
    (it "calls ensure-db! before replay"
      (with-redefs [launch-quil (stub :launch-quil)
                    game-logs/get-game-log (stub :game-log {:return {:game-id 2, :moves [[1 2 3 4 :x 6 7 8 9]], :second-ai-level 0, :mode 2, :first-ai-level 3, :game-state :in-progress, :ui :tui, :board [1 2 3 4 5 6 7 8 9]}})
                    sql/ensure-db! (stub :ensure-db)]
        (launch-user-interface ["gui" "--psqldb" "--game" "2"])
        (should-have-invoked :ensure-db)))
    ))
